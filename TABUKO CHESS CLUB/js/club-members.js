...
const ClubMembers = (() => {
  ...
  async function createMember(data) {
    ...
    if (data.fideId) {
      const existing = await db.collection('members').where('fideId', '==', data.fideId).get();
  ...
  async function importToTournament(tournamentId, playerId, config) {
    ...
    const defaultRating = config?.defaultRating || 1200;
    const clubRating = playerObj.ratings?.club || defaultRating;
    ...
    const tPlayer = {
      ...
      ratings: playerObj.ratings || { club: clubRating },
      selectedRating: clubRating,
      score: 0,
      isTemporary: false,
  ...
  return {
    createMember, getAllMembers, getMember, updateMember, removeMember,
