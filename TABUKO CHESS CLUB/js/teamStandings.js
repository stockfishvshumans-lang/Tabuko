  ...
  const computeBaseScores = (teams, rounds) => {
    ...
    rounds.forEach(round => {
      ...
      matches.forEach(m => {
        ...
        const t1BP = m.team1BP ?? m.homeBoardPoints ?? 0;
        const t2BP = m.team2BP ?? m.awayBoardPoints ?? 0;
        ...
        if (statsMap[t1Id]) {
          ...
          s.mp += t1MP;
          s.bp += t1BP;
          s.history.push({ round: rdNum, oppId: t2Id, mp: t1MP, bp: t1BP, isBye: false });
        ...
        if (statsMap[t2Id]) {
          ...
          s.mp += t2MP;
          s.bp += t2BP;
          s.history.push({ round: rdNum, oppId: t1Id, mp: t2MP, bp: t2BP, isBye: false });
      ...
      if (round.bye) {
        ...
        if (statsMap[tid]) {
          ...
          s.bp += (teamSize * 0.5); // Default FIDE bye BP (50%)
